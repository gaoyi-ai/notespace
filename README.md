```
├── Assembly
│   ├── Assembly-Lab 8.md
│   └── Assembly-Lab 9.md
├── BuildTools
│   └── Maven.md
├── DB
│   └── MySQL
│       └── MySQL.md
├── DSA
│   ├── Algorithm
│   │   ├── 0-1背包.md
│   │   ├── 1.两数之和.md
│   │   ├── 11.盛最多水的容器.md
│   │   ├── 113.路径总和Ⅱ.md
│   │   ├── 121.买卖股票的最佳时机.md
│   │   ├── 146.LRU缓存机制.md
│   │   ├── 15.三数之和.md
│   │   ├── 151.翻转字符串里的单词.md
│   │   ├── 155.最小栈.md
│   │   ├── 160.相交链表.md
│   │   ├── 2.两数相加.md
│   │   ├── 203.移除链表元素.md
│   │   ├── 22.括号生成.md
│   │   ├── 234.回文链表.md
│   │   ├── 239.滑动窗口最大值.md
│   │   ├── 242.有效的字母异位词.md
│   │   ├── 253.会议室II.md
│   │   ├── 283.移动零.md
│   │   ├── 3. 无重复字符的最长子串.md
│   │   ├── 333. 最大BST子树.md
│   │   ├── 39.组合总和.md
│   │   ├── 42.接雨水.md
│   │   ├── 46.全排列.md
│   │   ├── 47.全排列Ⅱ.md
│   │   ├── 5.最长回文子串(四种方法).md
│   │   ├── 50.Pow.md
│   │   ├── 54.螺旋矩阵.md
│   │   ├── 572.另一个树的子树.md
│   │   ├── 654.最大二叉树及变式.md
│   │   ├── 7.整数反转.md
│   │   ├── 72.编辑距离.md
│   │   ├── 739.每日温度.md
│   │   ├── 752.打开转盘锁.md
│   │   ├── 86.分割链表.md
│   │   ├── BellmanFord.md
│   │   ├── Bridge.md
│   │   ├── Cut.md
│   │   ├── DFS_BFS.md
│   │   ├── DNA.动态规划.md
│   │   ├── Flood_fill.pdf
│   │   ├── MST-Kruskal.md
│   │   ├── MST-Prim.md
│   │   ├── MaxSubseqSum.md
│   │   ├── Reverse Linklist.md
│   │   ├── Sequence.md
│   │   ├── ShortestPath-Dijkstra.md
│   │   ├── ShortestPath-Floyd.md
│   │   ├── collapse.动态规划.md
│   │   ├── hanoi.md
│   │   ├── topologicalSort.md
│   │   ├── 基数排序(次位优先)-Python.md
│   │   ├── 快速排序-Python.md
│   │   ├── 最长公共子串.md
│   │   ├── 计数排序-Python.md
│   │   ├── 迷宫探索—图的应用.md
│   │   ├── 面试题.62.圆圈中最后剩下的数字.md
│   │   ├── 面试题01.09.字符串轮转.md
│   │   ├── 面试题16.部分排序.md
│   │   └── 面试题47.礼物的最大价值.md
│   ├── DS
│   │   ├── AVL树-平衡二叉树.md
│   │   ├── B Tree.md
│   │   ├── B+ Tree.md
│   │   ├── BST-Binary Search Tree.md
│   │   ├── BloomFilter.md
│   │   ├── Fleury算法-欧拉图.md
│   │   ├── Graph Coloring.md
│   │   ├── Graph.md
│   │   ├── HashMap - 哈希表.md
│   │   ├── Havel算法.md
│   │   ├── Huffman Tree.md
│   │   ├── Red Black Tree - RBTree.md
│   │   ├── SkipList.md
│   │   ├── SplayTree.md
│   │   ├── Trie.md
│   │   ├── heap.md
│   │   ├── 二叉树遍历应用.md
│   │   ├── 哈密尔顿图.md
│   │   └── 并查集-Python.md
│   └── Visualization
│       └── AlgorithmAnimation.md
├── Discrete Mathematics
│   ├── Truth Table.md
│   ├── warshall算法.md
│   ├── 一元多项式相加.md
│   └── 后缀表达式.md
├── Django
│   ├── Django-Web安全防范.md
│   └── Django-model操作.md
├── ErrorCorrectCode
│   ├── HMCode.md
│   └── error-correct-code
│       ├── ErrorCorrectCode.iml
│       ├── LICENSE
│       ├── README.en.md
│       ├── README.md
│       ├── out
│       │   └── production
│       │       └── ErrorCorrectCode
│       │           └── algorithm
│       │               ├── Data.class
│       │               ├── HMData.class
│       │               ├── Parity.class
│       │               ├── ParityType.class
│       │               ├── Receiver.class
│       │               ├── Transceiver.class
│       │               ├── Transmitter.class
│       │               ├── Transmitting.class
│       │               └── Utils.class
│       └── src
│           └── algorithm
│               ├── Data.java
│               ├── HMData.java
│               ├── Parity.java
│               ├── ParityType.java
│               ├── Receiver.java
│               ├── Transceiver.java
│               ├── Transmitter.java
│               ├── Transmitting.java
│               └── Utils.java
├── Java
│   ├── Exception.md
│   ├── IO.md
│   ├── Integer.md
│   ├── Java Basic.md
│   ├── Java Immutable.md
│   ├── JavaGC.md
│   ├── JavaReference.md
│   ├── Reflection.md
│   ├── ServiceLoader.md
│   ├── String.md
│   ├── Understanding Network Class Loaders.md
│   ├── oop.md
│   ├── 依赖使用.md
│   ├── 对Java平台的理解.md
│   └── 持久层框架Demo.md
├── ML
│   ├── 01决策树
│   │   ├── Taitanic data.zip
│   │   ├── Tree
│   │   ├── Tree.dot
│   │   ├── Tree.pdf
│   │   ├── tree.jpg
│   │   ├── 决策树 full version.pdf
│   │   ├── 决策树 原理源码.ipynb
│   │   ├── 决策树 原理结果.html
│   │   ├── 决策树 案例源码.ipynb
│   │   ├── 决策树 案例结果.html
│   │   └── 决策树原理.pdf
│   └── 02随机森林
│       ├── digit recognizor.zip
│       ├── 随机森林 full version.pdf
│       ├── 随机森林 源码.html
│       └── 随机森林 源码.ipynb
├── Network
│   ├── OSPF与RIP协议对比.md
│   └── WebServer.md
├── Pattern
│   ├── Factory
│   │   ├── Java 工厂方法模式讲解和代码示例.pdf
│   │   ├── Java 抽象工厂模式讲解和代码示例.pdf
│   │   ├── 工厂方法设计模式.pdf
│   │   ├── 工厂模式和抽象工厂模式的区别.pdf
│   │   ├── 工厂模式详解.pdf
│   │   └── 抽象工厂设计模式.pdf
│   ├── Observer
│   │   ├── Java 观察者模式讲解和代码示例.pdf
│   │   ├── observer
│   │   │   ├── gupaoedu-vip-pattern-observer.iml
│   │   │   ├── pom.xml
│   │   │   └── src
│   │   │       ├── main
│   │   │       │   ├── java
│   │   │       │   │   ├── events
│   │   │       │   │   │   ├── MouseEventTest.java
│   │   │       │   │   │   ├── core
│   │   │       │   │   │   │   ├── Event.java
│   │   │       │   │   │   │   └── EventLisenter.java
│   │   │       │   │   │   └── mouseevent
│   │   │       │   │   │       ├── Mouse.java
│   │   │       │   │   │       ├── MouseEventCallback.java
│   │   │       │   │   │       └── MouseEventType.java
│   │   │       │   │   ├── gperadvice
│   │   │       │   │   │   ├── GPer.java
│   │   │       │   │   │   ├── ObserverTest.java
│   │   │       │   │   │   ├── Question.java
│   │   │       │   │   │   └── Teacher.java
│   │   │       │   │   └── guava
│   │   │       │   │       ├── GuavaEvent.java
│   │   │       │   │       └── GuavaEventTest.java
│   │   │       │   └── resources
│   │   │       └── test
│   │   │           └── java
│   │   ├── 装饰者模式及观察者模式.pdf
│   │   └── 观察者设计模式.pdf
│   ├── Proxy
│   │   ├── DynamicProxy.md
│   │   ├── Java 代理模式讲解和代码示例.pdf
│   │   ├── 代理设计模式.pdf
│   │   └── 深度分析代理模式.pdf
│   ├── Singleton
│   │   ├── Best Practices for Lazy Initialization.md
│   │   ├── Double-checked_locking.pdf
│   │   ├── Java 单例模式讲解和代码示例.pdf
│   │   ├── Singleton.md
│   │   ├── 单例模式详解.pdf
│   │   └── 单例设计模式.pdf
│   ├── Strategy
│   │   ├── Java 策略模式讲解和代码示例.pdf
│   │   ├── delegate
│   │   │   ├── gupaoedu-vip-pattern-delegate.iml
│   │   │   ├── pom.xml
│   │   │   └── src
│   │   │       ├── main
│   │   │       │   ├── java
│   │   │       │   │   ├── mvc
│   │   │       │   │   │   ├── DispatcherServlet.java
│   │   │       │   │   │   └── controllers
│   │   │       │   │   │       ├── MemberController.java
│   │   │       │   │   │       └── OrderController.java
│   │   │       │   │   └── simple
│   │   │       │   │       ├── Boss.java
│   │   │       │   │       ├── DelegateTest.java
│   │   │       │   │       ├── EmployeeA.java
│   │   │       │   │       ├── EmployeeB.java
│   │   │       │   │       ├── IEmployee.java
│   │   │       │   │       └── Leader.java
│   │   │       │   ├── resources
│   │   │       │   └── webapp
│   │   │       │       └── WEB-INF
│   │   │       │           └── web.xml
│   │   │       └── test
│   │   │           └── java
│   │   ├── strategy
│   │   │   ├── gupaoedu-vip-pattern-strategy.iml
│   │   │   ├── pom.xml
│   │   │   └── src
│   │   │       ├── main
│   │   │       │   ├── java
│   │   │       │   │   ├── pay
│   │   │       │   │   │   ├── MsgResult.java
│   │   │       │   │   │   ├── Order.java
│   │   │       │   │   │   ├── PayStrategyTest.java
│   │   │       │   │   │   └── payport
│   │   │       │   │   │       ├── AliPay.java
│   │   │       │   │   │       ├── PayStrategy.java
│   │   │       │   │   │       ├── Payment.java
│   │   │       │   │   │       └── WechatPay.java
│   │   │       │   │   └── promotion
│   │   │       │   │       ├── EmptyStrategy.java
│   │   │       │   │       ├── GroupbuyStrategy.java
│   │   │       │   │       ├── PromotionActivity.java
│   │   │       │   │       ├── PromotionActivityTest.java
│   │   │       │   │       ├── PromotionStrategy.java
│   │   │       │   │       └── PromotionStrategyFactory.java
│   │   │       │   └── resources
│   │   │       └── test
│   │   │           └── java
│   │   ├── 委派模式及策略模式.pdf
│   │   └── 策略设计模式.pdf
│   ├── Template
│   │   ├── Java 模版方法模式讲解和代码示例.pdf
│   │   └── 模板方法设计模式.pdf
│   └── 架构设计原则.pdf
├── Programming Paradigm
│   └── FourInLine - Functional Programming.md
├── Python
│   ├── Pythonic.md
│   ├── 协程.md
│   ├── 贪吃蛇-‘AI’锁头-Python.md
│   └── 进程与线程.md
├── README.md
├── SE
│   ├── CoC.md
│   ├── Convention over Configuration.pdf
│   ├── License.md
│   └── Pattern.md
└── WSN
    ├── WSN Q&A.md
    └── WSN.md

68 directories, 222 files
```


